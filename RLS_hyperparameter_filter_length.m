...
% Hyperparameter study of filter length
...
% Length of theta vector, the number of parameters 
% signal x_1 (N) and x_2 (M) are the same
p = 300;
p_jump = 10;
p_array = p_jump:p_jump:p;
...
for p = p_array
    N = p/2;
    M = p/2;
    disp([p, p_array(end)]) % print how many simuations are left
    ...
    q1_saved(p/p_jump) = q1;
    q2_saved(p/p_jump) = q2;
end
...
figure
hold on
plot(p_array, q1_saved, '-o')
plot(p_array, q2_saved, '-o')
xlabel("Filter length")
title("Hyperparameter study of filter length")
legend("Q1", "Q2")
